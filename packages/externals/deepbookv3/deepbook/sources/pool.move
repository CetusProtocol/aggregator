...
use deepbookv3::deep_price::DeepPrice;
...
public fun whitelisted<BaseAsset, QuoteAsset>(_self: &Pool<BaseAsset, QuoteAsset>): bool {
    abort 0
}

public fun registered_pool<BaseAsset, QuoteAsset>(_self: &Pool<BaseAsset, QuoteAsset>): bool {
    abort 0
}
...
/// Returns the pool ID.
public fun id<BaseAsset, QuoteAsset>(_self: &Pool<BaseAsset, QuoteAsset>): ID {
    abort 0
}
...
/// Check if a limit order can be placed based on pool book params and balance.
...
/// Check if a market order can be placed based on pool book params and balance.
...
/// Dry run to determine the quantity out for a given input quantity.
/// Uses input token as fee.
...
/// Permissionlessly withdraw settled amounts.
