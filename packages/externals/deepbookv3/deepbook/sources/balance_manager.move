...
use deepbookv3::registry::Registry;
...
/// DepositCap is used to deposit funds into a balance_manager by a non-owner.
...
/// Referral struct (deprecated).
public struct DeepBookReferral has key, store {
    id: UID,
    owner: address,
}

/// Pool-specific referral struct.
...
public fun mint_trade_cap(_balance_manager: &mut BalanceManager, _ctx: &mut TxContext): TradeCap {
    abort 0
}
...
public fun deposit<T>(_balance_manager: &mut BalanceManager, _coin: Coin<T>, _ctx: &mut TxContext) {
    abort 0
}
...
public fun withdraw_all<T>(_balance_manager: &mut BalanceManager, _ctx: &mut TxContext): Coin<T> {
    abort 0
}

public fun validate_proof(_balance_manager: &BalanceManager, _proof: &TradeProof) {
    abort 0
}
...
/// Create a new BalanceManager with custom owner caps.
...
/// Deposit with a DepositCap.
public fun deposit_with_cap<T>(
    _balance_manager: &mut BalanceManager,
    _deposit_cap: &DepositCap,
    _coin: Coin<T>,
    _ctx: &TxContext,
) {
    abort 0
...
/// Withdraw with a WithdrawCap.
public fun withdraw_with_cap<T>(
    _balance_manager: &mut BalanceManager,
    _withdraw_cap: &WithdrawCap,
    _withdraw_amount: u64,
    _ctx: &TxContext,
): Coin<T> {
    abort 0
...
/// Set balance manager referral.
public fun set_balance_manager_referral(
    _balance_manager: &mut BalanceManager,
    _referral_cap: &DeepBookPoolReferral,
    _trade_cap: &TradeCap,
) {
    abort 0
}

/// Unset balance manager referral.
